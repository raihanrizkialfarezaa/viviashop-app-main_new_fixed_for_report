...
class HomepageController extends Controller
{
    ...
    {
        $productActiveIds = Product::where(function($query) {
        ...
        $products = Product::with(['categories', 'productImages', 'productInventory'])
            ...
            ->limit(8)
            ->get();
        $categories = ProductCategory::with('products', 'categories')
            ->whereIn('product_id', $productActiveIds)
            ->get();
        $categoriesCount = ProductCategory::whereIn('product_id', $productActiveIds)->pluck('category_id');
