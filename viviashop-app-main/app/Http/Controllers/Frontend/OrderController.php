...
view()->share('setting', $setting);
	...
	/**
	 * TEST ONLY: Handle Midtrans notification without API validation
	 * For local testing with dummy data
	 */
	public function notificationHandlerTest(Request $request)
	{
		Log::info('TEST Midtrans Raw Notification Data:', $request->all());

		try {
			// Parse request data manually (no Midtrans SDK validation)
			$data = $request->all();
			...
			$transactionId = $data['transaction_id'] ?? 'TEST-' . time();
			...
			Log::info('TEST Midtrans Notification Received', [
				'order_id' => $orderId,
				'transaction_status' => $transaction,
				'payment_type' => $type,
				'fraud_status' => $fraud,
			...
			if (!$order) {
				Log::warning("TEST: Order not found for notification: {$orderId}");
				return response()->json([
					'success' => false,
					'message' => 'Order not found',
				], 404);
			...
			// Extract VA numbers for bank_transfer
			if ($type == 'bank_transfer' && isset($data['va_numbers']) && is_array($data['va_numbers']) && count($data['va_numbers']) > 0) {
				$paymentData['va_number'] = $data['va_numbers'][0]['va_number'] ?? null;
				$paymentData['va_bank'] = $data['va_numbers'][0]['bank'] ?? null;
			}

			// Extract echannel data
			if ($type == 'echannel') {
				$paymentData['bill_key'] = $data['bill_key'] ?? null;
				$paymentData['biller_code'] = $data['biller_code'] ?? null;
			}
			...
			switch ($transaction) {
				case 'capture':
					if ($type == 'credit_card') {
						if ($fraud == 'challenge') {
							$order->payment_status = Order::WAITING;
							$order->notes = $order->notes . "\n[TEST] Payment challenged by Fraud Detection System. Manual verification required.";
							Log::info("TEST: Order {$orderId} payment challenged by FDS");
						} else {
							$order->payment_status = Order::PAID;
							if ($order->shipping_service_name == 'Self Pickup') {
								$order->status = Order::CONFIRMED;
								$order->notes = $order->notes . "\n[TEST] Payment completed using credit card. Waiting for pickup confirmation.";
								Log::info("TEST: Order {$orderId} payment successful with credit card - self pickup order awaiting confirmation");
							} else {
								$order->status = Order::COMPLETED;
								$order->approved_at = now();
								$order->notes = $order->notes . "\n[TEST] Payment completed using credit card";
								Log::info("TEST: Order {$orderId} payment successful with credit card");
							}
						}
					}
					break;
				...
				case 'settlement':
					...
					if ($order->shipping_service_name == 'Self Pickup') {
						$order->status = Order::CONFIRMED;
						$order->notes = $order->notes . "\n[TEST] Payment settled using {$type}. Waiting for pickup confirmation.";
						Log::info("TEST: Order {$orderId} payment settled with {$type} - self pickup order awaiting confirmation");
					} else {
						$order->status = Order::COMPLETED;
						$order->approved_at = now();
						$order->notes = $order->notes . "\n[TEST] Payment settled using {$type}";
						Log::info("TEST: Order {$orderId} payment settled with {$type}");
					}
					break;

				case 'pending':
					$order->payment_status = Order::WAITING;
					$order->notes = $order->notes . "\n[TEST] Payment pending using {$type}";
					Log::info("TEST: Order {$orderId} payment pending with {$type}");
					break;

				case 'deny':
					$order->payment_status = Order::CANCELLED;
					$order->status = Order::CANCELLED;
					$order->notes = $order->notes . "\n[TEST] Payment denied using {$type}";
					Log::warning("TEST: Order {$orderId} payment denied with {$type}");
					$this->_restoreStock($order);
					break;
				...
				case 'expire':
					$order->payment_status = Order::CANCELLED;
					$order->status = Order::CANCELLED;
					$order->notes = $order->notes . "\n[TEST] Payment expired for {$type}";
					Log::warning("TEST: Order {$orderId} payment expired for {$type}");
					$this->_restoreStock($order);
					break;
				...
				case 'cancel':
					$order->payment_status = Order::CANCELLED;
					$order->status = Order::CANCELLED;
					$order->notes = $order->notes . "\n[TEST] Payment canceled for {$type}";
					Log::warning("TEST: Order {$orderId} payment canceled for {$type}");
					$this->_restoreStock($order);
					break;

				default:
					Log::error("TEST: Unknown transaction status: {$transaction} for order {$orderId}");
					break;
			}
			...
			try {
				\App\Models\Payment::create(array_merge($paymentData, ['order_id' => $order->id]));
				Log::info("TEST: Payment record created for order {$orderId}");
			} catch (\Exception $e) {
				Log::warning("TEST: Failed to create payment record for order {$orderId}: " . $e->getMessage());
			}
			...
			Log::error('TEST Midtrans Notification Error: ' . $e->getMessage());
			Log::error($e->getTraceAsString());
			return response()->json([
				'success' => false,
				'message' => 'Error processing notification: ' . $e->getMessage(),
