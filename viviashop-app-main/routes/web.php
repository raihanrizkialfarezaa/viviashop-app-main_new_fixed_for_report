...
// TEST ENDPOINT - for local testing with dummy data (bypasses Midtrans API validation)
Route::post('payments/notification-test', [App\Http\Controllers\Frontend\OrderController::class, 'notificationHandlerTest'])
...
Route::group(['middleware' => ['auth', 'is_admin'], 'prefix' => 'admin', 'as' => 'admin.'], function() {
    ...
    Route::get('reports/revenue/{awal}/{akhir}/pdf', [\App\Http\Controllers\Admin\ReportController::class, 'revenuePdfWithDates'])->name('reports.revenue.pdf-dates');
    Route::get('reports/product', [\App\Http\Controllers\Admin\ReportController::class, 'product'])->name('reports.product');
    Route::get('reports/product/pdf', [\App\Http\Controllers\Admin\ReportController::class, 'productPdf'])->name('reports.product.pdf');
    Route::get('reports/test-pdf', [\App\Http\Controllers\Admin\ReportController::class, 'testPdfUI'])->name('reports.test-pdf');
    Route::post('reports/test-pdf/run', [\App\Http\Controllers\Admin\ReportController::class, 'runPdfTest'])->name('reports.test-pdf.run');
    ...
    Route::prefix('stock-opname')->as('stock-opname.')->group(function() {
        ...
        Route::get('/{productId}/variants', [\App\Http\Controllers\Admin\StockOpnameController::class, 'variants'])->name('variants');
        Route::post('/process', [\App\Http\Controllers\Admin\StockOpnameController::class, 'process'])->name('process');
    });
...
// Public / frontend chat (UC1 + UC2) — no auth required for chat itself;
// individual tools enforce their own role check at execution time.
Route::middleware('web')->prefix('ai')->group(function () {
    Route::post('/chat', [\App\Http\Controllers\AIAgentController::class, 'handleChat'])
        ->name('ai.chat')
        ->middleware('throttle:' . config('ai.agent.rate_limit', 30) . ',1');

    // File upload for print assistant — requires auth
    Route::post('/upload', [\App\Http\Controllers\AIAgentController::class, 'uploadAttachment'])
        ->name('ai.upload')
        ->middleware('auth');
});

// Admin AI assistant console (UC3 + UC4)
Route::middleware(['auth', 'is_admin'])
    ->prefix('admin/ai-assistant')
    ->group(function () {
        Route::get('/', [\App\Http\Controllers\AIAgentController::class, 'adminConsole'])
            ->name('admin.ai.console');

        Route::post('/chat', [\App\Http\Controllers\AIAgentController::class, 'handleAdminChat'])
            ->name('admin.ai.chat')
            ->middleware('throttle:' . config('ai.agent.rate_limit', 30) . ',1');
    });
